...
public sealed class BookController : ControllerBase
{
  ...
  [HttpPost]
  public async Task<IActionResult> Add([FromBody] Dtos.BookAddReq request)
  {
    var grpcRequest = request.ToBookAddRequest();
    return Ok(await _client.AddBookAsync(grpcRequest));
  }

  [HttpPut]
  public async Task<IActionResult> Update([FromBody] Dtos.BookUpdReq request)
  {
    var grpcRequest = request.ToBookUpdateRequest();
    return Ok(await _client.UpdateBookAsync(grpcRequest));
  }
  ...
  [HttpGet("hasfile/{id:Guid}")]
  public async Task<IActionResult> HasFile([FromRoute] Guid id)
  {
    return Ok(await _client.HasFileAsync(new Common.HasFileRequest()
    {
      Id = id.ToString()
    }));
  }

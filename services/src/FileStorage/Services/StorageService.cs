...
public sealed class StorageService : FileStorageService.FileStorageServiceBase
{
  ...
  public override async Task<UploadReply> Upload(IAsyncStreamReader<FileChunkMessage> requestStream, ServerCallContext context)
  {
    ...
    try
    {
      ...
      var fileNameWithoutExt = Path.GetFileNameWithoutExtension(it.FileName).Trim();
      if (fileNameWithoutExt.EndsWith('.'))
      {
        fileNameWithoutExt.Remove(fileNameWithoutExt.Length - 1);
      }
      if (fileNameWithoutExt.Length > 100)
        fileNameWithoutExt = fileNameWithoutExt[..100];
      var fileName = $"[{date.Year}-{date.Month}-{date.Day}] ({uuid}) {fileNameWithoutExt}.{fileExtension}";
  ...
  public override async Task<Empty> DeleteEntry(DeleteEntryRequest request, ServerCallContext context)
  {
    if (!Guid.TryParse(request.Uuid, out Guid id))
      throw new RpcException(new Status(StatusCode.InvalidArgument, "The provided uuid is not Guid."));
    var found = await _dbContext.FileEntries.FindAsync(id)
                ?? throw new RpcException(new Status(StatusCode.NotFound, $"The file entry with id = {id} not found."));
    if (File.Exists(found.FilePath))
    {
